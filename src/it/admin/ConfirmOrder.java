	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 try {
				con = connessionedb.createConnection();
				 String dlt ="UPDATE ordini_user SET status = '3' WHERE id = '"+ idcart +"' ";
				 st = con.createStatement();
				 st.executeUpdate(dlt);
		         con.close();
	             request.setAttribute("Message", "<div id=\"ServletMessage\" class=\"container center_div\" style=\" padding:15px;\">\n" + 
	             		"	<div class=\"alert alert-success\" role=\"alert\"> L'ordine è arrivato a destinazione!</div>  </div>  "); 
				...
				request.setAttribute("errorMessage", "Conferma ordine non ha avuto successo!");

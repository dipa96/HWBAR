	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 try {
				con = connessionedb.createConnection();
				 String dlt ="UPDATE ordini_user SET status = '5' WHERE id = '"+ idcart +"' ";
				 st = con.createStatement();
				 st.executeUpdate(dlt);
		         con.close();
	             request.setAttribute("Message", "<div id=\"ServletMessage\" class=\"container center_div\" style=\" padding:15px;\">\n" + 
	             		"	<div class=\"alert alert-success\" role=\"alert\"> L'ordine è stato annullato con successo! </div>  </div>  "); 
				...
				request.setAttribute("errorMessage", "Non è possibile annullare l'ordine!");
